...
fn parse_field(data: &[u8]) -> Result<(Field, &[u8]), Error> {
    ...
    let (fieldvalue, remainder) = match wire_type {
        WireType::Varint => {
            let (v, r) = parse_varint(remainder)?;
        ...
        WireType::Len => {
            ...
            (FieldValue::Len(&rem[..len as usize]), &rem[len as usize..])
        }
        WireType::I32 => {
            if remainder.len() < 4 {
                return Err(Error::UnexpectedEOF);
            ...
            (
                FieldValue::I32(i32::from_le_bytes([
                    remainder[0],
                    remainder[1],
                    remainder[2],
                    remainder[3],
    ...
    fn add_field(&mut self, field: Field<'a>) -> Result<(), Error> {
        ...
        match &field.value {
            ...
            Len(_) => {
                ...
                let mut phone_number: PhoneNumber = Default::default();
                let (number, r) = parse_field(field.value.as_bytes()?)?;
                let (type_, _) = parse_field(r)?;
                phone_number.add_field(number)?;
                phone_number.add_field(type_)?;
...
impl<'a> ProtoMessage<'a> for PhoneNumber<'a> {
    fn add_field(&mut self, field: Field<'a>) -> Result<(), Error> {
        ...
        match &field.value {
            Len(_) => {
                println!("phone len called: {}", &self.number);

                let value = field.value.as_string()?;
